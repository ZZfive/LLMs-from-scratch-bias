...
# Define a grid of hyperparameters to search over  参数搜索空间
HPARAM_GRID = {
    "batch_size": [2, 4, 8, 16],
...
def calc_loss_loader(data_loader, model, device, num_batches=None):  # 针对数据加载器计算损失
    total_loss = 0.
...
def calc_loss_batch(input_batch, target_batch, model, device):  # 计算较交叉熵损失
