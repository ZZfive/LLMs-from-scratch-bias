...
本补充材料说明了如何在 GPT 模型训练设置中使用 PyTorch 的 Muon 优化器。

&nbsp;
## 引言

Muon（["Muon 可扩展用于 LLM 训练"](https://arxiv.org/abs/2502.16982)）是一种相对较新的优化器，用于训练 LLM 中占据 Transformer 块主导地位的大型 2D 权重矩阵，例如注意力投影、前馈投影和输出头。不过，不适合 Muon 目标的参数，如嵌入、偏置和归一化参数，通常仍保留在 AdamW 上。

具体来说，这意味着：

1. 对不属于嵌入层的可训练 2D 参数使用 Muon。
2. 对嵌入、偏置、归一化参数和其他非 2D 参数使用 AdamW。
3. 在学习率方面，例如，尝试 Muon 从 `lr=1e-4` 开始，AdamW 从 `lr=5e-5` 开始，`weight_decay=0.1`，并且 Muon 使用 `adjust_lr_fn="match_rms_adamw"`。
...
替代的 [gpt_train_muon.py](gpt_train_muon.py) 脚本从相同的模型实现开始，但使用了 Muon（以及 AdamW）。

我建议查看 [gpt_train.py](gpt_train.py) 和 [gpt_train_muon.py](gpt_train_muon.py) 之间的文件差异，以快速了解这里是如何实现 Muon 的。
...
顺便说一句，这并不是有意作为一个有意义的语言建模基准。模型是随机初始化的，并且仅在微小的重复文本片段上训练了一个 epoch，以便易于检查优化器路径且运行快速。
